...
class NPC(AnimatedSprite):
    ...
    def check_hit_in_npc(self):
        # Instead of using ray_cast_value, check if the NPC is on screen and being aimed at
        if self.game.player.shot and self.ray_cast_value:
            # Check if the mouse/fire is aimed at this NPC
            # The sprite is on screen and the crosshair is over it
            if (HALF_WIDTH - self.sprite_half_width < self.screen_x < HALF_WIDTH + self.sprite_half_width 
                and self.distance < 10):  # Add a max distance check
                print(f"NPC HIT! Health: {self.health}")  # Debug print
        ...
        if self.alive:
            ...
            if self.pain:
                ...
                if self.distance < self.attack_dist:
                    # Only play attack animation when close enough to attack
                    self.animate(self.attack_images)
                    self.attack()
                else:
                    # Only walk when not in attack range
                    self.animate(self.walk_images)
                    self.movement() 
                    
            elif self.player_search_trigger:
                # Searching for player (walking)
                self.animate(self.walk_images)
                self.movement() 
            else:
